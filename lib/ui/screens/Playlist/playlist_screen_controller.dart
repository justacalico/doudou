...
class PlaylistScreenController extends PlaylistAlbumScreenControllerBase
  ...
  Future<void> _fetchSongOnline(
    ...
    final content =
        await backend.getPlaylistOrAlbumSongs(playlistId: id);
  ...
  Future<bool> checkIfAddedToLibrary(String id) async {
    ...
    final key = libraryPlaylistKey(currentServerId(), id);
    isAddedToLibrary.value = box.containsKey(key);
    if (isAddedToLibrary.value) {
      playlist.value = Playlist.fromJson(
  ...
  Future<bool> addNremoveFromLibrary(dynamic content, {bool add = true}) async {
    ...
    try {
      if (content.isPipedPlaylist && !add) {
        ...
        final key = libraryPlaylistKey(currentServerId(), id);
        if (add) {
          box.put(key, content.toJson());
          updateSongsIntoDb();
        } else {
          box.delete(key);
          if (id != "LIBFAV" && id != "LIBRP") {
            final songsBox = await Hive.openBox(playlistSongsBoxName(id));
      ...
      if (!content.isCloudPlaylist && !add && content.playlistId != "LIBFAV") {
        final plstbox =
            await Hive.openBox(playlistSongsBoxName(content.playlistId));
  ...
  Future<void> updateSongsIntoDb() async {
    final songsBox =
        await Hive.openBox(playlistSongsBoxName(playlist.value.playlistId));
  ...
  Future<void> deleteMultipleSongs(List<MediaItem> songs) async {
    ...
    final box_ = await Hive.openBox(playlistSongsBoxName(id));
  ...
  Future<void> exportPlaylistToJson(BuildContext context) async {
    ...
    try {
      ...
      String errorMsg = l10n.exportError;
      if (e is FileSystemException) {
        if (e.osError?.errorCode == 13) {
          errorMsg = l10n.exportErrorPermission;
        } else if (e.osError?.errorCode == 28) {
          errorMsg = l10n.exportErrorStorage;
        }
      } else if (e is FormatException) {
        errorMsg = l10n.exportErrorFormat;
      }
