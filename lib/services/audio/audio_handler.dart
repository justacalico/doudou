import 'dart:async';import 'dart:async';import 'dart:async';

import 'dart:io';

import 'package:audio_service/audio_service.dart';import 'dart:io';import 'dart:io';

import 'package:just_audio/just_audio.dart';

import 'package:flutter/foundation.dart';import 'package:audio_service/audio_service.dart';import 'package:audio_service/audio_service.dart';

import 'package:rxdart/rxdart.dart';

import '../../models/jellyfin_models.dart';import 'package:just_audio/just_audio.dart';import 'package:just_audio/just_audio.dart';

import '../jellyfin_service.dart';

import '../download_service.dart';import 'package:flutter/foundation.dart';import 'package:flutter/foundation.dart';



/// Clean, modern audio handler that avoids race conditions and complex synchronizationimport 'package:rxdart/rxdart.dart';import 'package:flutter/material.dart';

class DoudouAudioHandler extends BaseAudioHandler with QueueHandler, SeekHandler {

  final AudioPlayer _player = AudioPlayer();import '../../models/jellyfin_models.dart';import '../../models/jellyfin_models.dart';

  final JellyfinService _jellyfinService;

  final DownloadService _downloadService;import '../jellyfin_service.dart';import '../jellyfin_service.dart';



  // Simple state management - no complex mutexes or coordinatorsimport '../download_service.dart';import '../media_service_manager.dart';

  bool _isInitialized = false;

  List<Track> _playlist = [];import '../download_service.dart';

  int? _currentIndex;

  bool _shuffleEnabled = false;/// Clean, modern audio handler that avoids race conditions and complex synchronizationimport '../touchbar_service.dart';

  AudioServiceRepeatMode _repeatMode = AudioServiceRepeatMode.none;

class DoudouAudioHandler extends BaseAudioHandler with QueueHandler, SeekHandler {import '../lyrics_service.dart';

  // Stream controllers for custom state

  final BehaviorSubject<Map<String, dynamic>> _customStateController =   final AudioPlayer _player = AudioPlayer();import '../logging_service.dart';

      BehaviorSubject<Map<String, dynamic>>.seeded({});

  final JellyfinService _jellyfinService;import 'audio_state_manager.dart';

  Stream<Map<String, dynamic>> get customState => _customStateController.stream;

  final DownloadService _downloadService;import 'audio_preloader.dart';
...
import 'audio_queue_manager.dart';
  ...
  Future<void> initializeDirectly() async {  // Simple state management - no complex mutexes or coordinatorsimport 'audio_radio_mode.dart';

    if (_isInitialized) return;

      bool _isInitialized = false;import 'audio_state_persistence.dart';

    try {

      await _initialize();  List<Track> _playlist = [];import 'audio_transition_manager.dart';

      _isInitialized = true;

        int? _currentIndex;import 'async_mutex.dart';

      if (kDebugMode) {

        print('DoudouAudioHandler: Initialized directly for ${Platform.operatingSystem}');  bool _shuffleEnabled = false;import 'jellyfin_service_coordinator.dart';

      }

    } catch (e) {  AudioServiceRepeatMode _repeatMode = AudioServiceRepeatMode.none;import 'error_state_manager.dart';

      if (kDebugMode) {

        print('DoudouAudioHandler: Failed to initialize directly: $e');import 'audio_session_coordinator.dart';

      }

      rethrow;  // Stream controllers for custom stateimport 'player_state_transition_coordinator.dart';

    }

  }  final BehaviorSubject<Map<String, dynamic>> _customStateController = import 'audio_operation_queue.dart';



  /// Common initialization logic      BehaviorSubject<Map<String, dynamic>>.seeded({});import 'audio_state_machine.dart';

  Future<void> _initialize() async {

    // Set up player listenersimport 'operation_cancellation.dart';

    _setupPlayerListeners();

  Stream<Map<String, dynamic>> get customState => _customStateController.stream;import 'android_service_manager.dart';

    // Initialize audio session based on platform

    await _initializeAudioSession();import 'audio_position_manager.dart';



    // Set initial playback state  DoudouAudioHandler(this._jellyfinService, this._downloadService);import 'state_persistence_manager.dart';

    playbackState.add(PlaybackState(

      controls: [import 'radio_mode_state_manager.dart';

        MediaControl.skipToPrevious,

        MediaControl.play,  /// Initialize the audio handler directly (without AudioService)import 'audio_lifecycle_manager.dart';

        MediaControl.skipToNext,

      ],  Future<void> initializeDirectly() async {import 'touchbar_update_manager.dart';

      systemActions: const {

        MediaAction.seek,    if (_isInitialized) return;import 'download_service_coordinator.dart';

        MediaAction.seekForward,

        MediaAction.seekBackward,    import 'media_service_manager_coordinator.dart';

      },

      androidCompactActionIndices: const [0, 1, 2],    try {

      processingState: AudioProcessingState.idle,

      playing: false,      await _initialize();class DoudouAudioHandler extends BaseAudioHandler with QueueHandler, SeekHandler {

      queueIndex: 0,

    ));      _isInitialized = true;  final AudioPlayer _player = AudioPlayer();



    if (kDebugMode) {        final JellyfinService _jellyfinService;

      print('DoudouAudioHandler: Audio session initialized for ${Platform.operatingSystem}');

    }      if (kDebugMode) {  final DownloadService _downloadService;

  }

        print('DoudouAudioHandler: Initialized directly for ${Platform.operatingSystem}');  final MediaServiceManager? _mediaServiceManager;
  ...
  Future<void> _initializeAudioSession() async {      }  
    ...
    try {
      ...
      if (Platform.isIOS) {    } catch (e) {  // Android foreground service management with immutable state

        // iOS audio session configuration

        await _player.setAudioSource(      if (kDebugMode) {  final AndroidServiceManager _androidServiceManager = AndroidServiceManager();
        ...
        // Android audio focus configuration      }  // Touch Bar service for macOS

        await _player.setAudioSource(

          ConcatenatingAudioSource(children: []),      rethrow;  bool _touchBarEnabled = false;
    ...
    } catch (e) {  }  // Lyrics state for TouchBar display

      if (kDebugMode) {

        print('DoudouAudioHandler: Audio session setup warning: $e');  LyricsResult? _currentLyrics;

      }

    }  /// Common initialization logic  int _currentLyricsLineIndex = -1;

  }

  Future<void> _initialize() async {  String? _lastLyricsTrackId;

  /// Set up player event listeners

  void _setupPlayerListeners() {    // Set up player listeners  

    // Listen to player state changes

    _player.playerStateStream.listen((playerState) {    _setupPlayerListeners();  // Background track completion watchdog for Android
    ...
    // Listen to position changes    // Initialize audio session based on platform  DateTime? _lastTrackPositionUpdate;

    _player.positionStream.listen((position) {

      if (playbackState.value.updatePosition != position) {    await _initializeAudioSession();  Duration? _lastKnownDuration;
    ...
    // Set initial playback state  // Component managers

    // Listen to duration changes

    _player.durationStream.listen((duration) {    playbackState.add(PlaybackState(  late final AudioStateManager _stateManager;

      _updateCurrentMediaItem();

    });      controls: [  late final AudioPreloader _preloader;



    // Listen to current index changes (for gapless playback)        MediaControl.skipToPrevious,  late final AudioQueueManager _queueManager;

    _player.currentIndexStream.listen((index) {

      if (index != null && index != _currentIndex) {        MediaControl.play,  late final AudioRadioMode _radioMode;

        _currentIndex = index;

        _updateCurrentMediaItem();        MediaControl.skipToNext,  late final AudioStatePersistence _statePersistence;
    ...
    _player.playbackEventStream.listen((event) {        MediaAction.seek,  // State persistence with debouncing to prevent file corruption

      if (event.processingState == ProcessingState.idle && 

          _currentIndex != null &&         MediaAction.seekForward,  late final StatePersistenceManager _statePersistenceManager;

          _playlist.isNotEmpty) {

        _handlePlaybackError();        MediaAction.seekBackward,  
    ...
    if (kDebugMode) {      androidCompactActionIndices: const [0, 1, 2],  late final RadioModeStateManager _radioModeStateManager;

      print('DoudouAudioHandler: Player listeners set up');

    }      processingState: AudioProcessingState.idle,  late final RadioModeOperationManager _radioModeOperationManager;
  ...
  void _updatePlaybackState() {      queueIndex: 0,  // Touch Bar update synchronization for macOS to prevent visual glitches
    ...
    playbackState.add(PlaybackState(
      ...
      systemActions: const {
        ...
        MediaAction.seek,    if (kDebugMode) {  // Download service coordination to prevent interference with audio streaming

        MediaAction.seekForward,

        MediaAction.seekBackward,      print('DoudouAudioHandler: Audio session initialized for ${Platform.operatingSystem}');  late final DownloadServiceCoordinator _downloadServiceCoordinator;
      ...
      playing: playerState.playing,  }  // Media service manager coordination to prevent disposal race conditions

      updatePosition: _player.position,

      bufferedPosition: _player.bufferedPosition,  late final MediaServiceManagerCoordinator? _mediaServiceManagerCoordinator;

      speed: _player.speed,

      queueIndex: _currentIndex,  /// Initialize platform-specific audio session

      shuffleMode: _shuffleEnabled ? AudioServiceShuffleMode.all : AudioServiceShuffleMode.none,

      repeatMode: _repeatMode,  Future<void> _initializeAudioSession() async {  // Jellyfin service coordination to prevent API timeout race conditions

    ));

  }    try {  late final JellyfinServiceCoordinator _jellyfinServiceCoordinator;



  /// Get appropriate media controls based on playing state      if (Platform.isIOS) {

  List<MediaControl> _getMediaControls(bool isPlaying) {

    return [        // iOS audio session configuration  // Centralized error state management to prevent conflicting error handling

      MediaControl.skipToPrevious,

      isPlaying ? MediaControl.pause : MediaControl.play,        await _player.setAudioSource(  late final ErrorStateManager _errorStateManager;
  ...
  AudioProcessingState _mapProcessingState(ProcessingState state) {      } else if (Platform.isAndroid) {  late final AudioSessionCoordinator _audioSessionCoordinator;

    switch (state) {

      case ProcessingState.idle:        // Android audio focus configuration

        return AudioProcessingState.idle;

      case ProcessingState.loading:        await _player.setAudioSource(  // Player state transition coordination to prevent state machine race conditions

        return AudioProcessingState.loading;

      case ProcessingState.buffering:          ConcatenatingAudioSource(children: []),  late final PlayerStateTransitionCoordinator _playerStateTransitionCoordinator;
      ...
      case ProcessingState.completed:      }  // App lifecycle management to handle Android background restrictions

        return AudioProcessingState.completed;

    }      // Other platforms use default configuration  late final AudioLifecycleManager _lifecycleManager;
  ...
  void _updateCurrentMediaItem() {      if (kDebugMode) {  // Media browsing data for Android Auto

    if (_currentIndex != null && 

        _currentIndex! >= 0 &&         print('DoudouAudioHandler: Audio session setup warning: $e');  List<Album> _albums = [];

        _currentIndex! < _playlist.length) {

      final track = _playlist[_currentIndex!];      }  List<Artist> _artists = [];
  ...
  void _updateCustomState() {
    ...
    _customStateController.add({
      ...
      'shuffle': _shuffleEnabled,  /// Set up player event listeners  // Gapless playback with ConcatenatingAudioSource

      'repeat': _repeatMode.index,

      'currentIndex': _currentIndex,  void _setupPlayerListeners() {  ConcatenatingAudioSource? _concatenatingSource;

      'playlistLength': _playlist.length,

    });    // Listen to player state changes  bool _isUsingConcatenation = false;

  }

    _player.playerStateStream.listen((playerState) {  final Map<String, AudioSource> _audioSourceCache = {};

  /// Handle playback errors with simple recovery

  void _handlePlaybackError() {      _updatePlaybackState();

    if (kDebugMode) {

      print('DoudouAudioHandler: Handling playback error, attempting recovery');    });  // Logging service

    }

  final LoggingService _logger = LoggingService();

    // Simple error recovery - try to reload current track

    Future.delayed(const Duration(seconds: 1), () async {    // Listen to position changes

      if (_currentIndex != null && _currentIndex! < _playlist.length) {

        try {    _player.positionStream.listen((position) {  // RACE CONDITION PROTECTION: Modern async synchronization

          await _playTrackAtIndex(_currentIndex!);

        } catch (e) {      if (playbackState.value.updatePosition != position) {  // Replace custom spinlocks with proper async mutex

          if (kDebugMode) {

            print('DoudouAudioHandler: Error recovery failed: $e');        _updatePlaybackState();  final NamedMutexManager _mutexManager = NamedMutexManager();
  ...
  MediaItem _createMediaItem(Track track) {    // Listen to duration changes  

    return MediaItem(

      id: track.id,    _player.durationStream.listen((duration) {  // State machine for synchronized player state and user intent

      title: track.name,

      artist: track.artistName ?? 'Unknown Artist',      _updateCurrentMediaItem();  final AudioStateMachine _stateMachine = AudioStateMachine();
      ...
      playable: true,  // Cancellation manager for preventing operation race conditions

      extras: {

        'trackId': track.id,    // Listen to current index changes (for gapless playback)  final OperationCancellationManager _cancellationManager = OperationCancellationManager();

        'albumId': track.albumId,

        'trackNumber': track.trackNumber,    _player.currentIndexStream.listen((index) {  

      },

    );      if (index != null && index != _currentIndex) {  // Position manager for atomic position updates

  }

        _currentIndex = index;  final AudioPositionManager _positionManager = AudioPositionManager();

  /// Build image URI for track artwork

  Uri? _buildImageUri(String imageId) {        _updateCurrentMediaItem();

    try {

      // Use JellyfinService to build proper image URL        _updateCustomState();  // Legacy user intent tracking - will be replaced by state machine

      final imageUrl = _jellyfinService.getImageUrl(imageId, ImageType.primary);

      return Uri.parse(imageUrl);      }  bool _userIntendedPlaying = false;

    } catch (e) {

      if (kDebugMode) {    });  bool _userExplicitlyPaused = false; // Track intentional user pause
    ...
    _player.playbackEventStream.listen((event) {  // Position tracking for pause/resume to prevent position jumping

  /// Get stream URL for a track with fallbacks

  Future<String?> _getStreamUrl(Track track) async {      if (event.processingState == ProcessingState.idle &&   Duration? _pausedAtPosition;

    try {

      // Check for downloaded file first          _currentIndex != null &&   

      final localPath = await _downloadService.getLocalFilePath(track.id);

      if (localPath != null && await File(localPath).exists()) {          _playlist.isNotEmpty) {  // Playlist operation debouncing

        return 'file://$localPath';

      }        _handlePlaybackError();  DateTime? _lastPlaylistOperation;
    ...
    } catch (e) {    });  // Codec loop detection with synchronized access

      if (kDebugMode) {

        print('DoudouAudioHandler: Failed to get stream URL for ${track.name}: $e');  DateTime? _lastBufferingTime;

      }

      return null;    if (kDebugMode) {  int _bufferingLoopCount = 0;
  ...
  /// Play a track at the specified index    }  // Volume state protection

  Future<void> _playTrackAtIndex(int index) async {

    if (index < 0 || index >= _playlist.length) return;  }  double? _previousVolume;
    ...
    try {  
      ...
      final track = _playlist[index];  /// Update the playback state based on current player state  // Lyrics state protection - no longer needs locks with proper state management

      

      if (kDebugMode) {  void _updatePlaybackState() {  

        print('DoudouAudioHandler: Playing track at index $index: ${track.name}');

      }    final playerState = _player.playerState;  // Completion handling protection - managed by transition manager
      ...
      if (streamUrl == null) {    playbackState.add(PlaybackState(  // Track loading protection to prevent concurrent loads causing "Loading interrupted"

        throw Exception('No stream URL available for track: ${track.name}');

      }      controls: _getMediaControls(playerState.playing),  // Now handled by operation queue



      // Load and play the track      systemActions: const {  

      if (streamUrl.startsWith('file://')) {

        await _player.setFilePath(streamUrl.substring(7));        MediaAction.seek,  // Helper methods for safe state management using async mutex

      } else {

        await _player.setUrl(streamUrl);        MediaAction.seekForward,  Future<void> _setConcatenationState(bool usingConcatenation, [ConcatenatingAudioSource? source]) async {

      }

        MediaAction.seekBackward,    return await _mutexManager.withLock('concatenationState', () async {

      // Update media item and state

      _updateCurrentMediaItem();      },      _isUsingConcatenation = usingConcatenation;

      _updateCustomState();

      _updatePlaybackState();      androidCompactActionIndices: const [0, 1, 2],      _concatenatingSource = source;



      if (kDebugMode) {      processingState: _mapProcessingState(playerState.processingState),    });
      ...
      if (kDebugMode) {      updatePosition: _player.position,  

        print('DoudouAudioHandler: Failed to play track at index $index: $e');

      }      bufferedPosition: _player.bufferedPosition,  Future<bool> _isConcatenationActive() async {

      

      // Update state to show error      speed: _player.speed,    return await _mutexManager.withLock('concatenationState', () async {

      playbackState.add(playbackState.value.copyWith(

        processingState: AudioProcessingState.error,      queueIndex: _currentIndex,      return _isUsingConcatenation && _concatenatingSource != null;
  ...
  Future<void> play() async {  }  // Helper method for debounced state persistence to prevent file corruption

    try {

      if (_playlist.isEmpty) {  void _savePlaybackStateDebounced({Duration? position, bool? isPlaying}) {

        if (kDebugMode) {

          print('DoudouAudioHandler: Cannot play - no tracks in playlist');  /// Get appropriate media controls based on playing state    if (position != null && isPlaying != null) {

        }

        return;  List<MediaControl> _getMediaControls(bool isPlaying) {      // Create specific save function for provided state

      }

    return [      final specificManager = StatePersistenceManager(

      if (_player.audioSource == null && _currentIndex != null) {

        // Load current track if not loaded      MediaControl.skipToPrevious,        saveFunction: () => _statePersistence.savePlaybackState(position, isPlaying),

        await _playTrackAtIndex(_currentIndex!);

      }      isPlaying ? MediaControl.pause : MediaControl.play,        debounceDelay: const Duration(milliseconds: 300),



      await _player.play();      MediaControl.skipToNext,      );

      

      if (kDebugMode) {    ];      specificManager.requestSave();
      ...
      if (kDebugMode) {      // Use general manager for current player state

        print('DoudouAudioHandler: Play failed: $e');

      }  /// Map just_audio ProcessingState to audio_service AudioProcessingState      _statePersistenceManager.requestSave();
  ...
  Future<void> pause() async {
    ...
    try {      case ProcessingState.idle:  

      await _player.pause();

              return AudioProcessingState.idle;  Future<void> _clearAudioSourceCache() async {

      if (kDebugMode) {

        print('DoudouAudioHandler: Pause command executed');      case ProcessingState.loading:    return await _mutexManager.withLock('audioSourceCache', () async {

      }

    } catch (e) {        return AudioProcessingState.loading;      _audioSourceCache.clear();

      if (kDebugMode) {

        print('DoudouAudioHandler: Pause failed: $e');      case ProcessingState.buffering:    });
  ...
  Future<void> stop() async {        return AudioProcessingState.ready;  Future<void> _setUserIntentAtomic(bool intendedPlaying) async {

    try {

      await _player.stop();      case ProcessingState.completed:    return await _mutexManager.withLock('userIntent', () async {

      

      if (kDebugMode) {        return AudioProcessingState.completed;      if (kDebugMode) {
  ...
  }  /// Update the current media item      final previousIntent = _userIntendedPlaying;



  @override  void _updateCurrentMediaItem() {      _userIntendedPlaying = intendedPlaying;

  Future<void> seek(Duration position) async {

    try {    if (_currentIndex != null &&       

      await _player.seek(position);

              _currentIndex! >= 0 &&       // Clear explicit pause flag when user intends to play

      if (kDebugMode) {

        print('DoudouAudioHandler: Seek to ${position.inSeconds}s');        _currentIndex! < _playlist.length) {      if (intendedPlaying) {

      }

    } catch (e) {      final track = _playlist[_currentIndex!];        _userExplicitlyPaused = false;

      if (kDebugMode) {

        print('DoudouAudioHandler: Seek failed: $e');      mediaItem.add(_createMediaItem(track));      }
  ...
  Future<void> skipToNext() async {      _audioSessionCoordinator.setUserIntendedPlaying(intendedPlaying);

    try {

      if (_currentIndex == null || _playlist.isEmpty) return;  /// Update custom state (for UI synchronization)      



      int nextIndex;  void _updateCustomState() {      // Update state machine intent

      if (_shuffleEnabled) {

        // Random next track (excluding current)    _customStateController.add({      if (intendedPlaying) {

        final availableIndices = List.generate(_playlist.length, (i) => i)

            .where((i) => i != _currentIndex)      'shuffle': _shuffleEnabled,        _stateMachine.setIntent(UserIntent.play);

            .toList();

        if (availableIndices.isEmpty) return;      'repeat': _repeatMode.index,      } else {

        nextIndex = availableIndices[DateTime.now().millisecondsSinceEpoch % availableIndices.length];

      } else {      'currentIndex': _currentIndex,        _stateMachine.setIntent(UserIntent.pause);
        ...
        if (nextIndex >= _playlist.length) {    });      
          ...
          if (_repeatMode == AudioServiceRepeatMode.all) {
            ...
            nextIndex = 0;  }      // Mobile optimization: Reduce verification delay for faster response

          } else {

            return; // End of playlist      if (previousIntent != intendedPlaying) {

          }

        }  /// Handle playback errors with simple recovery        final isMobile = Platform.isAndroid || Platform.isIOS;

      }

  void _handlePlaybackError() {        final verificationDelay = isMobile ? 25 : 100; // Reduce mobile delay from 100ms to 25ms

      await _playTrackAtIndex(nextIndex);

      await play();    if (kDebugMode) {        



      if (kDebugMode) {      print('DoudouAudioHandler: Handling playback error, attempting recovery');        if (kDebugMode && isMobile) {
      ...
      if (kDebugMode) {            print('Mobile optimized user intent: Using ${verificationDelay}ms verification delay');
  ...
  @override      if (_currentIndex != null && _currentIndex! < _playlist.length) {        

  Future<void> skipToPrevious() async {

    try {        try {        Future.delayed(Duration(milliseconds: verificationDelay), () {

      if (_currentIndex == null || _playlist.isEmpty) return;

          await _playTrackAtIndex(_currentIndex!);          _verifyUserIntentAlignment();

      // If we're more than 3 seconds in, just restart current track

      if (_player.position.inSeconds > 3) {        } catch (e) {        });
        ...
        return;          if (kDebugMode) {      }
            ...
            print('DoudouAudioHandler: Error recovery failed: $e');      

      int previousIndex;

      if (_shuffleEnabled) {          }      if (kDebugMode) {

        // Random previous track (excluding current)

        final availableIndices = List.generate(_playlist.length, (i) => i)        }        print('User intent updated successfully: $intendedPlaying');
        ...
        if (previousIndex < 0) {
          ...
          if (_repeatMode == AudioServiceRepeatMode.all) {  /// Create a MediaItem from a Track  /// Verifies that the actual player state aligns with user intent and corrects if needed

            previousIndex = _playlist.length - 1;

          } else {  MediaItem _createMediaItem(Track track) {  Future<void> _verifyUserIntentAlignment() async {

            previousIndex = 0; // Stay at first track

          }    return MediaItem(    // Skip verification during transitions to avoid interference

        }

      }      id: track.id,    if (_transitionManager.isTransitionInProgress || _stateManager.isHandlingCompletion) {



      await _playTrackAtIndex(previousIndex);      title: track.name,      return;
      ...
      if (kDebugMode) {
        ...
        print('DoudouAudioHandler: Skipped to previous track at index $previousIndex');      album: track.albumName ?? 'Unknown Album',    

      }

    } catch (e) {      duration: track.duration != null ? Duration(milliseconds: track.duration!) : null,    final playerPlaying = _player.playing;

      if (kDebugMode) {

        print('DoudouAudioHandler: Skip to previous failed: $e');      artUri: track.imageUrl != null ? _buildImageUri(track.imageUrl!) : null,    final userWantsPlaying = _userIntendedPlaying && !_userExplicitlyPaused;

      }

    }      playable: true,    final processingState = _player.processingState;
  ...
  Future<void> skipToQueueItem(int index) async {        'trackId': track.id,    // Only verify for ready state to avoid interfering with loading/buffering

    try {

      if (index >= 0 && index < _playlist.length) {        'albumId': track.albumId,    if (processingState != ProcessingState.ready) {

        await _playTrackAtIndex(index);

        await play();        'trackNumber': track.trackNumber,      return;
    ...
    } catch (e) {  }    if (playerPlaying != userWantsPlaying) {

      if (kDebugMode) {

        print('DoudouAudioHandler: Skip to queue item failed: $e');      if (kDebugMode) {

      }

    }  /// Build image URI for track artwork        print('USER INTENT MISALIGNMENT: Player=$playerPlaying, UserWants=$userWantsPlaying, Intended=$_userIntendedPlaying, ExplicitPause=$_userExplicitlyPaused');
  ...
  Future<void> updateQueue(List<MediaItem> queue) async {    try {      

    try {

      // Convert MediaItems back to Tracks      // Use JellyfinService to build proper image URL      try {

      _playlist = queue.map((item) => Track(

        id: item.id,      final imageUrl = _jellyfinService.getImageUrl(imageId, ImageType.primary);        if (userWantsPlaying && !playerPlaying) {

        name: item.title,

        artistName: item.artist,      return Uri.parse(imageUrl);          if (kDebugMode) {

        albumName: item.album,

        duration: item.duration?.inMilliseconds,    } catch (e) {            print('Correcting: Starting playback to align with user intent');

        albumId: item.extras?['albumId'] as String?,

        trackNumber: item.extras?['trackNumber'] as int?,      if (kDebugMode) {          }

        imageUrl: item.artUri?.toString(),

      )).toList();        print('DoudouAudioHandler: Failed to build image URI: $e');          await _player.play();



      // Reset current index      }        } else if (!userWantsPlaying && playerPlaying) {

      _currentIndex = _playlist.isNotEmpty ? 0 : null;

      return null;          if (kDebugMode) {

      // Update the queue

      super.queue.add(queue);    }            print('Correcting: Pausing playback to align with user intent');
      ...
      if (kDebugMode) {
        ...
        print('DoudouAudioHandler: Updated queue with ${_playlist.length} tracks');          await _player.pause();

      }

    } catch (e) {  /// Get stream URL for a track with fallbacks        }

      if (kDebugMode) {

        print('DoudouAudioHandler: Update queue failed: $e');  Future<String?> _getStreamUrl(Track track) async {      } catch (e) {
      ...
      // Check for downloaded file first          print('Failed to correct user intent alignment: $e');

  @override

  Future<void> addQueueItem(MediaItem mediaItem) async {      final localPath = await _downloadService.getLocalFilePath(track.id);        }

    try {

      final track = Track(      if (localPath != null && await File(localPath).exists()) {      }

        id: mediaItem.id,

        name: mediaItem.title,        return 'file://$localPath';    }
        ...
        imageUrl: mediaItem.artUri?.toString(),      // Get streaming URL from Jellyfin  

      );

      return _jellyfinService.getStreamUrl(track.id);  Future<void> _updateBufferingLoop() async {

      _playlist.add(track);

          } catch (e) {    return await _mutexManager.withLock('bufferingState', () async {

      // If this is the first track, set it as current

      _currentIndex ??= 0;      if (kDebugMode) {      final now = DateTime.now();



      // Update the queue        print('DoudouAudioHandler: Failed to get stream URL for ${track.name}: $e');      if (_lastBufferingTime != null && 

      final currentQueue = List<MediaItem>.from(queue.value ?? []);

      currentQueue.add(mediaItem);      }          now.difference(_lastBufferingTime!) < const Duration(seconds: 10)) {

      super.queue.add(currentQueue);

      _updateCustomState();      return null;        _bufferingLoopCount++;
      ...
      }  /// Play a track at the specified index      _lastBufferingTime = now;
  ...
  @override    if (index < 0 || index >= _playlist.length) return;  }
  ...
  Future<void> removeQueueItemAt(int index) async {
    ...
    try {  
      ...
      if (index >= 0 && index < _playlist.length) {
        ...
        final removedTrack = _playlist.removeAt(index);    try {  Future<bool> _shouldHandleCodecLoop() async {

        

        // Adjust current index if needed      _currentIndex = index;    return await _mutexManager.withLock('bufferingState', () async {

        if (_currentIndex != null) {

          if (index < _currentIndex!) {      final track = _playlist[index];      return _bufferingLoopCount >= 25;
            ...
            if (_currentIndex! >= _playlist.length) {      if (kDebugMode) {  }
            ...
            if (_playlist.isNotEmpty) {
              ...
              await _playTrackAtIndex(_currentIndex!);      }  Future<void> _resetBufferingLoop() async {
      ...
      // Get stream URL      _bufferingLoopCount = 0;

        // Update the queue

        final currentQueue = List<MediaItem>.from(queue.value ?? []);      final streamUrl = await _getStreamUrl(track);      _lastBufferingTime = null;

        if (index < currentQueue.length) {

          currentQueue.removeAt(index);      if (streamUrl == null) {    });
        ...
        if (kDebugMode) {
          ...
          print('DoudouAudioHandler: Removed track from queue: ${removedTrack.name}');  // Helper method to completely clear and reset player state

        }

      }      // Load and play the track  Future<void> _resetPlayerStateCompletely() async {

    } catch (e) {

      if (kDebugMode) {      if (streamUrl.startsWith('file://')) {    return await _playerOperationQueue.enqueue('resetPlayerState', () async {

        print('DoudouAudioHandler: Remove queue item failed: $e');

      }        await _player.setFilePath(streamUrl.substring(7));      _logger.info('Completely resetting player state', 'AudioHandler');
  ...
  @override        await _player.setUrl(streamUrl);      // Set flag to prevent recovery interference during intentional reset

  Future<void> setShuffleMode(AudioServiceShuffleMode shuffleMode) async {

    _shuffleEnabled = shuffleMode == AudioServiceShuffleMode.all;      }      _isIntentionallyResetting = true;
    ...
    if (kDebugMode) {      // Update media item and state      try {

      print('DoudouAudioHandler: Shuffle mode set to: $shuffleMode');

    }      _updateCurrentMediaItem();        // Optimize for mobile: Reduce delays for faster queue switching but keep them stable

  }

      _updateCustomState();        final isMobile = Platform.isAndroid || Platform.isIOS;

  @override

  Future<void> setRepeatMode(AudioServiceRepeatMode repeatMode) async {      _updatePlaybackState();        final stopDelay = isMobile ? 150 : 200; // Conservative mobile delay for MediaCodec stability

    _repeatMode = repeatMode;

    _updateCustomState();        final finalDelay = isMobile ? 200 : 300; // Conservative mobile delay for MediaCodec stability
    ...
    if (kDebugMode) {
      ...
      print('DoudouAudioHandler: Repeat mode set to: $repeatMode');        print('DoudouAudioHandler: Successfully loaded track: ${track.name}');        if (kDebugMode && isMobile) {
  ...
  /// Dispose of resources    } catch (e) {            print('Mobile optimized reset: Using conservative delays (${stopDelay}ms + ${finalDelay}ms)');

  void dispose() {

    _player.dispose();      if (kDebugMode) {          }
...
enum ImageType {            // Stop player first and wait for it to fully stop

  primary,

  backdrop,      // Update state to show error      await _errorStateManager.executeWithErrorHandling(

  banner,

  logo,      playbackState.add(playbackState.value.copyWith(        component: 'AudioHandler',

}
        processingState: AudioProcessingState.error,        operation: 'stopPlayer',

        playing: false,        category: ErrorCategory.playback,
  ...
  // AudioHandler implementation          // Wait for player to fully stop and release all resources

  @override          await Future.delayed(Duration(milliseconds: stopDelay));

  Future<void> play() async {          

    try {          _logger.info('Player stopped successfully', 'AudioHandler');

      if (_playlist.isEmpty) {        },

        if (kDebugMode) {        context: {'operation': 'resetPlayerState'},

          print('DoudouAudioHandler: Cannot play - no tracks in playlist');      );

        }      

        return;      // Clear all caches and state atomically

      }      await _clearAudioSourceCache();

      await _setConcatenationState(false, null);

      if (_player.audioSource == null && _currentIndex != null) {      _preloader.clearAllPreloadedPlayers();

        // Load current track if not loaded      

        await _playTrackAtIndex(_currentIndex!);      // Reset transition states

      }      await _transitionManager.waitForTransitionComplete();

      _stateManager.setHandlingCompletion(false);

      await _player.play();      _stateManager.setTransitioning(false);

            

      if (kDebugMode) {      // CRITICAL FIX: Clear the current track reference to prevent old track confusion

        print('DoudouAudioHandler: Play command executed');      _stateManager.setCurrentTrack(null);

      }      

    } catch (e) {      // Mobile optimization: Reduce final delay for faster response

      if (kDebugMode) {      await Future.delayed(Duration(milliseconds: finalDelay));
  ...
  Future<void> pause() async {    });
    ...
    try {  }
      ...
      if (kDebugMode) {  void _updatePlaybackState(PlaybackState newState) {

        print('DoudouAudioHandler: Pause command executed');    // Mobile optimization: Reduce debug logging overhead for better performance

      }    final isMobile = Platform.isAndroid || Platform.isIOS;

    } catch (e) {    final shouldDebugState = kDebugMode && !isMobile; // Disable verbose state logging on mobile

      if (kDebugMode) {    

        print('DoudouAudioHandler: Pause failed: $e');    if (shouldDebugState) {
  ...
  @override        print('DateTime: ${DateTime.now()}');

  Future<void> stop() async {      }

    try {      if (kDebugMode) {

      await _player.stop();        print('Input newState.playing: ${newState.playing}');

            }

      if (kDebugMode) {      if (kDebugMode) {

        print('DoudouAudioHandler: Stop command executed');        print('Input newState.processingState: ${newState.processingState}');

      }      }

    } catch (e) {      if (kDebugMode) {

      if (kDebugMode) {        print('Current _userIntendedPlaying: $_userIntendedPlaying');
  ...
  @override        print('Previous playbackState.playing: ${playbackState.value.playing}');
  ...
  Future<void> seek(Duration position) async {      }
    ...
    try {    }
      ...
      if (kDebugMode) {    
        ...
        print('DoudouAudioHandler: Seek to ${position.inSeconds}s');    // Simplified state synchronization - prioritize user intent over complex logic

      }    final shouldBePlaying = _getUserIntentedPlayState(newState);

    } catch (e) {    

      if (kDebugMode) {    // Update state machine based on processing state

        print('DoudouAudioHandler: Seek failed: $e');    switch (newState.processingState) {
  ...
  @override      case AudioProcessingState.ready:

  Future<void> skipToNext() async {        _stateMachine.transitionTo(AudioPlayerState.ready);

    try {        _positionManager.setBuffering(false);

      if (_currentIndex == null || _playlist.isEmpty) return;        // For ready state, ensure UI reflects user intent

        finalState = newState.copyWith(playing: shouldBePlaying);

      int nextIndex;        break;

      if (_shuffleEnabled) {      case AudioProcessingState.buffering:

        // Random next track (excluding current)        _stateMachine.transitionTo(AudioPlayerState.buffering);

        final availableIndices = List.generate(_playlist.length, (i) => i)        _positionManager.setBuffering(true);

            .where((i) => i != _currentIndex)        // During buffering, maintain user intent in UI

            .toList();        finalState = newState.copyWith(playing: shouldBePlaying);

        if (availableIndices.isEmpty) return;        break;

        nextIndex = availableIndices[DateTime.now().millisecondsSinceEpoch % availableIndices.length];      case AudioProcessingState.completed:

      } else {        _stateMachine.transitionTo(AudioPlayerState.completed);

        nextIndex = _currentIndex! + 1;        _positionManager.setBuffering(false);

                // Track completed - respect that unless radio mode continues

        // Handle repeat modes        finalState = newState.copyWith(playing: false);

        if (nextIndex >= _playlist.length) {        break;

          if (_repeatMode == AudioServiceRepeatMode.all) {      case AudioProcessingState.error:

            nextIndex = 0;        _stateMachine.transitionTo(AudioPlayerState.error);

          } else {        _positionManager.setBuffering(false);

            return; // End of playlist        // Reset user intent when player enters error state
      ...
      await _playTrackAtIndex(nextIndex);        _stateMachine.transitionTo(AudioPlayerState.idle);

      await play();        _positionManager.setBuffering(false);
    ...
    } catch (e) {    // Verify actual player state alignment with intended state

      if (kDebugMode) {    _verifyPlayerStateAlignment(finalState);
  ...
  @override          print('=== FINAL PLAYBACK STATE UPDATE ===');

  Future<void> skipToPrevious() async {        }

    try {        if (kDebugMode) {

      if (_currentIndex == null || _playlist.isEmpty) return;          print('finalState.playing: ${finalState.playing}');

        }

      // If we're more than 3 seconds in, just restart current track        if (kDebugMode) {

      if (_player.position.inSeconds > 3) {          print('finalState.processingState: ${finalState.processingState}');
      ...
      int previousIndex;        if (kDebugMode) {

      if (_shuffleEnabled) {          print('About to call playbackState.add(finalState)');
        ...
        final availableIndices = List.generate(_playlist.length, (i) => i)      }
            ...
            .toList();      playbackState.add(finalState);

        if (availableIndices.isEmpty) return;      

        previousIndex = availableIndices[DateTime.now().millisecondsSinceEpoch % availableIndices.length];      if (shouldDebugState) {

      } else {        if (kDebugMode) {

        previousIndex = _currentIndex! - 1;          print('Successfully updated playbackState stream');

                }

        // Handle repeat modes        if (kDebugMode) {

        if (previousIndex < 0) {          print('=== END _updatePlaybackState ===');
          ...
          if (_repeatMode == AudioServiceRepeatMode.all) {        }
            ...
            previousIndex = 0; // Stay at first track      if (shouldDebugState) {
      ...
      await _playTrackAtIndex(previousIndex);          print('Error updating playback state: $e');
      ...
      if (kDebugMode) {      
        ...
        print('DoudouAudioHandler: Skipped to previous track at index $previousIndex');      // Handle the error through the service manager (Android-specific)

      }      if (Platform.isAndroid) {

    } catch (e) {        final newConfig = _androidServiceManager.handlePlaybackStateError(e);

      if (kDebugMode) {        

        print('DoudouAudioHandler: Skip to previous failed: $e');        if (shouldDebugState) {
  ...
  @override            print('New service state: ${newConfig.description}');
  ...
  Future<void> skipToQueueItem(int index) async {          }
    ...
    try {        }
      ...
      if (index >= 0 && index < _playlist.length) {        
        ...
        await _playTrackAtIndex(index);        // If we transitioned to bypass mode, skip further AudioService operations

        await play();        if (newConfig.shouldBypass) {

          if (shouldDebugState) {

        if (kDebugMode) {            if (kDebugMode) {

          print('DoudouAudioHandler: Skipped to queue item at index $index');              print('Android service manager: Bypassing AudioService for future operations');
      ...
      if (kDebugMode) {          if (shouldDebugState) {
  ...
  Future<void> updateQueue(List<MediaItem> queue) async {          }
    ...
    try {          
      ...
      // Convert MediaItems back to Tracks          // Fallback: Try without media controls for Android service issues

      _playlist = queue.map((item) => Track(          try {

        id: item.id,            final fallbackState = finalState.copyWith(

        name: item.title,              controls: [], // Remove controls that might trigger foreground service

        artistName: item.artist,              systemActions: const <MediaAction>{}, // Remove system actions

        albumName: item.album,            );

        duration: item.duration?.inMilliseconds,            playbackState.add(fallbackState);

        albumId: item.extras?['albumId'] as String?,            

        trackNumber: item.extras?['trackNumber'] as int?,            if (kDebugMode) {

        imageUrl: item.artUri?.toString(),              print('Fallback playback state update successful');

      )).toList();            }

          } catch (fallbackError) {

      // Reset current index            if (kDebugMode) {

      _currentIndex = _playlist.isNotEmpty ? 0 : null;              print('Fallback playback state update also failed: $fallbackError');
      ...
      if (kDebugMode) {    
        ...
        print('DoudouAudioHandler: Updated queue with ${_playlist.length} tracks');    // Update Touch Bar with new playback state

      }    if (Platform.isMacOS) {

    } catch (e) {      _updateTouchBarPlaybackState();
  ...
  @override    if (_userExplicitlyPaused) {

  Future<void> addQueueItem(MediaItem mediaItem) async {      return false;

    try {    }

      final track = Track(    

        id: mediaItem.id,    // For ready and buffering states, use user intended playing state

        name: mediaItem.title,    if (newState.processingState == AudioProcessingState.ready ||

        artistName: mediaItem.artist,        newState.processingState == AudioProcessingState.buffering) {

        albumName: mediaItem.album,      return _userIntendedPlaying;

        duration: mediaItem.duration?.inMilliseconds,    }

        albumId: mediaItem.extras?['albumId'] as String?,    

        trackNumber: mediaItem.extras?['trackNumber'] as int?,    // For loading state, maintain user intent if available

        imageUrl: mediaItem.artUri?.toString(),    if (newState.processingState == AudioProcessingState.loading) {
      ...
      // If this is the first track, set it as current    return false;

      _currentIndex ??= 0;  }

  

      // Update the queue  /// Verifies that player state aligns with the intended state and triggers recovery if needed

      final currentQueue = List<MediaItem>.from(queue.value ?? []);  void _verifyPlayerStateAlignment(PlaybackState finalState) {

      currentQueue.add(mediaItem);    // Only check alignment for ready state to avoid interference during transitions

      super.queue.add(currentQueue);    if (finalState.processingState != AudioProcessingState.ready) {

      _updateCustomState();      return;

    }

      if (kDebugMode) {    

        print('DoudouAudioHandler: Added track to queue: ${track.name}');    // Check if player state and UI state are misaligned

      }    final playerActuallyPlaying = _player.playing;

    } catch (e) {    final uiShouldShowPlaying = finalState.playing;

      if (kDebugMode) {    

        print('DoudouAudioHandler: Add queue item failed: $e');    if (playerActuallyPlaying != uiShouldShowPlaying) {
  ...
  Future<void> removeQueueItemAt(int index) async {      // Schedule a recovery attempt after a delay - longer on mobile to avoid conflicts with intentional resets

    try {      final recoveryDelay = Platform.isAndroid || Platform.isIOS ? 1200 : 500;

      if (index >= 0 && index < _playlist.length) {      Future.delayed(Duration(milliseconds: recoveryDelay), () {

        final removedTrack = _playlist.removeAt(index);        _attemptStateRecovery(uiShouldShowPlaying);
        ...
        if (_currentIndex != null) {  }
          ...
          if (index < _currentIndex!) {  
            ...
            _currentIndex = _currentIndex! - 1;  /// Attempts to recover from state misalignment

          } else if (index == _currentIndex!) {  Future<void> _attemptStateRecovery(bool shouldBePlaying) async {

            // Removed current track, play next or previous    // Don't attempt recovery if we're intentionally resetting the player

            if (_currentIndex! >= _playlist.length) {    if (_isIntentionallyResetting) {

              _currentIndex = _playlist.length - 1;      if (kDebugMode) {

            }        print('State recovery skipped - intentional reset in progress');

            if (_playlist.isNotEmpty) {      }

              await _playTrackAtIndex(_currentIndex!);      return;
    ...
    final currentlyPlaying = _player.playing;
        ...
        // Update the queue    if (currentlyPlaying == shouldBePlaying) {

        final currentQueue = List<MediaItem>.from(queue.value ?? []);      return; // Already aligned

        if (index < currentQueue.length) {    }

          currentQueue.removeAt(index);    

          super.queue.add(currentQueue);    if (kDebugMode) {
        ...
        if (kDebugMode) {    try {
          ...
          print('DoudouAudioHandler: Removed track from queue: ${removedTrack.name}');      if (shouldBePlaying && !currentlyPlaying && _userIntendedPlaying && !_userExplicitlyPaused) {

        }        await _player.play();

      }        if (kDebugMode) {

    } catch (e) {          print('Recovery: Started playback');

      if (kDebugMode) {        }

        print('DoudouAudioHandler: Remove queue item failed: $e');      } else if (!shouldBePlaying && currentlyPlaying && (!_userIntendedPlaying || _userExplicitlyPaused)) {
  ...
  Future<void> setShuffleMode(AudioServiceShuffleMode shuffleMode) async {    } catch (e) {

    _shuffleEnabled = shuffleMode == AudioServiceShuffleMode.all;      if (kDebugMode) {

    _updateCustomState();        print('State recovery failed: $e');
  ...
  Future<void> setRepeatMode(AudioServiceRepeatMode repeatMode) async {    _stateManager = AudioStateManager();

    _repeatMode = repeatMode;    _preloader = AudioPreloader(_jellyfinService, _downloadService);

    _updateCustomState();    _queueManager = AudioQueueManager(_stateManager);

    _updatePlaybackState();    _radioMode = AudioRadioMode(_jellyfinService);

    _statePersistence = AudioStatePersistence(_stateManager);

    if (kDebugMode) {    _transitionManager = AudioTransitionManager();
  ...
  /// Dispose of resources        // Save current player state when no position/playing state is provided

  void dispose() {        final position = _player.position;

    _player.dispose();        final isPlaying = _player.playing;

    _customStateController.close();        await _statePersistence.savePlaybackState(position, isPlaying);
...
enum ImageType {    // Initialize synchronized radio mode state management

  primary,    _radioModeStateManager = RadioModeStateManager();

  backdrop,    _radioModeOperationManager = RadioModeOperationManager(_radioModeStateManager);

  banner,    

  logo,    // Initialize synchronized Touch Bar update manager for macOS
  ...
  Future<void> pause() async {
    ...
    // Stop background watchdog when user explicitly pauses
  ...
  Future<void> _playCurrentTrack() async {
    ...
    await _downloadServiceCoordinator.markTrackAsStreaming(actualTrack.id);
    
    if (kDebugMode) {
      print('Playing track ${_stateManager.currentIndex + 1}/${_stateManager.playlist.length}: ${actualTrack.name}');
    }
