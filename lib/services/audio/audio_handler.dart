...
import '../../models/jellyfin_models.dart';import '../../models/jellyfin_models.dart';

import '../jellyfin_service.dart';import '../jellyfin_service.dart';

import '../download_service.dart';import '../media_service_manager.dart';

import '../download_service.dart';

/// Clean, modern audio handler that avoids race conditions and complex synchronizationimport '../touchbar_service.dart';

class DoudouAudioHandler extends BaseAudioHandler with QueueHandler, SeekHandler {import '../lyrics_service.dart';

  final AudioPlayer _player = AudioPlayer();import '../logging_service.dart';
...
import 'audio_queue_manager.dart';
  ...
  // Simple state management - no complex mutexes or coordinatorsimport 'audio_radio_mode.dart';

  bool _isInitialized = false;import 'audio_state_persistence.dart';

  List<Track> _playlist = [];import 'audio_transition_manager.dart';

  int? _currentIndex;import 'async_mutex.dart';

  bool _shuffleEnabled = false;import 'jellyfin_service_coordinator.dart';

  AudioServiceRepeatMode _repeatMode = AudioServiceRepeatMode.none;import 'error_state_manager.dart';

import 'audio_session_coordinator.dart';

  // Stream controllers for custom stateimport 'player_state_transition_coordinator.dart';

  final BehaviorSubject<Map<String, dynamic>> _customStateController = import 'audio_operation_queue.dart';

      BehaviorSubject<Map<String, dynamic>>.seeded({});import 'audio_state_machine.dart';
...
import 'audio_position_manager.dart';
  ...
  DoudouAudioHandler(this._jellyfinService, this._downloadService);import 'state_persistence_manager.dart';

import 'radio_mode_state_manager.dart';

  /// Initialize the audio handler directly (without AudioService)import 'audio_lifecycle_manager.dart';

  Future<void> initializeDirectly() async {import 'touchbar_update_manager.dart';

    if (_isInitialized) return;import 'download_service_coordinator.dart';

    import 'media_service_manager_coordinator.dart';

    try {

      await _initialize();class DoudouAudioHandler extends BaseAudioHandler with QueueHandler, SeekHandler {
    ...
    } catch (e) {  // Android foreground service management with immutable state

      if (kDebugMode) {  final AndroidServiceManager _androidServiceManager = AndroidServiceManager();

        print('DoudouAudioHandler: Failed to initialize directly: $e');  

      }  // Touch Bar service for macOS

      rethrow;  bool _touchBarEnabled = false;
  ...
  Future<void> _initialize() async {  String? _lastLyricsTrackId;
    ...
    _setupPlayerListeners();  // Background track completion watchdog for Android

  Timer? _backgroundWatchdog;

    // Initialize audio session based on platform  DateTime? _lastTrackPositionUpdate;

    await _initializeAudioSession();  Duration? _lastKnownDuration;

  

    // Set initial playback state  // Component managers

    playbackState.add(PlaybackState(  late final AudioStateManager _stateManager;

      controls: [  late final AudioPreloader _preloader;

        MediaControl.skipToPrevious,  late final AudioQueueManager _queueManager;

        MediaControl.play,  late final AudioRadioMode _radioMode;

        MediaControl.skipToNext,  late final AudioStatePersistence _statePersistence;

      ],  late final AudioTransitionManager _transitionManager;

      systemActions: const {  

        MediaAction.seek,  // State persistence with debouncing to prevent file corruption

        MediaAction.seekForward,  late final StatePersistenceManager _statePersistenceManager;

        MediaAction.seekBackward,  

      },  // Radio mode state synchronization to prevent UI/streaming inconsistencies 

      androidCompactActionIndices: const [0, 1, 2],  late final RadioModeStateManager _radioModeStateManager;

      processingState: AudioProcessingState.idle,  late final RadioModeOperationManager _radioModeOperationManager;

      playing: false,  

      queueIndex: 0,  // Touch Bar update synchronization for macOS to prevent visual glitches

    ));  late final TouchBarUpdateManager _touchBarUpdateManager;

  

    if (kDebugMode) {  // Download service coordination to prevent interference with audio streaming

      print('DoudouAudioHandler: Audio session initialized for ${Platform.operatingSystem}');  late final DownloadServiceCoordinator _downloadServiceCoordinator;
  ...
  Future<void> _initializeAudioSession() async {  // Jellyfin service coordination to prevent API timeout race conditions

    try {  late final JellyfinServiceCoordinator _jellyfinServiceCoordinator;

      if (Platform.isIOS) {

        // iOS audio session configuration  // Centralized error state management to prevent conflicting error handling

        await _player.setAudioSource(  late final ErrorStateManager _errorStateManager;

          ConcatenatingAudioSource(children: []),

        );  // iOS audio session coordination to prevent race conditions

      } else if (Platform.isAndroid) {  late final AudioSessionCoordinator _audioSessionCoordinator;

        // Android audio focus configuration

        await _player.setAudioSource(  // Player state transition coordination to prevent state machine race conditions

          ConcatenatingAudioSource(children: []),  late final PlayerStateTransitionCoordinator _playerStateTransitionCoordinator;
      ...
      if (kDebugMode) {  // Media browsing data for Android Auto

        print('DoudouAudioHandler: Audio session setup warning: $e');  List<Album> _albums = [];
  ...
  /// Set up player event listeners  // Gapless playback with ConcatenatingAudioSource

  void _setupPlayerListeners() {  ConcatenatingAudioSource? _concatenatingSource;
  ...
  final LoggingService _logger = LoggingService();
    ...
    _player.positionStream.listen((position) {  // RACE CONDITION PROTECTION: Modern async synchronization

      if (playbackState.value.updatePosition != position) {  // Replace custom spinlocks with proper async mutex

        _updatePlaybackState();  final NamedMutexManager _mutexManager = NamedMutexManager();
  ...
  final AudioOperationQueue _playerOperationQueue = AudioOperationQueue();
    ...
    _player.durationStream.listen((duration) {  // State machine for synchronized player state and user intent

      _updateCurrentMediaItem();  final AudioStateMachine _stateMachine = AudioStateMachine();

    });  

  // Cancellation manager for preventing operation race conditions

    // Listen to current index changes (for gapless playback)  final OperationCancellationManager _cancellationManager = OperationCancellationManager();
    ...
    _player.currentIndexStream.listen((index) {  
      ...
      if (index != null && index != _currentIndex) {  // Position manager for atomic position updates
        ...
        _updateCustomState();  // Legacy user intent tracking - will be replaced by state machine
  ...
  bool _isIntentionallyResetting = false; // Track when we're intentionally resetting to prevent recovery interference
    ...
    _player.playbackEventStream.listen((event) {  // Position tracking for pause/resume to prevent position jumping
      ...
      if (event.processingState == ProcessingState.idle &&   Duration? _pausedAtPosition;

          _currentIndex != null &&   

          _playlist.isNotEmpty) {  // Playlist operation debouncing
  ...
  DateTime? _lastBufferingTime;
    ...
    if (kDebugMode) {  int _bufferingLoopCount = 0;
  ...
  /// Update the playback state based on current player state  // Lyrics state protection - no longer needs locks with proper state management
  ...
  void _updatePlaybackState() {  
    ...
    playbackState.add(PlaybackState(  // Track loading protection to prevent concurrent loads causing "Loading interrupted"

      controls: _getMediaControls(playerState.playing),  // Now handled by operation queue

      systemActions: const {  

        MediaAction.seek,  // Helper methods for safe state management using async mutex

        MediaAction.seekForward,  Future<void> _setConcatenationState(bool usingConcatenation, [ConcatenatingAudioSource? source]) async {
      ...
      androidCompactActionIndices: const [0, 1, 2],      _concatenatingSource = source;
      ...
      speed: _player.speed,    return await _mutexManager.withLock('concatenationState', () async {

      queueIndex: _currentIndex,      return _isUsingConcatenation && _concatenatingSource != null;
  ...
  /// Get appropriate media controls based on playing state    if (position != null && isPlaying != null) {

  List<MediaControl> _getMediaControls(bool isPlaying) {      // Create specific save function for provided state

    return [      final specificManager = StatePersistenceManager(

      MediaControl.skipToPrevious,        saveFunction: () => _statePersistence.savePlaybackState(position, isPlaying),
  ...
  AudioProcessingState _mapProcessingState(ProcessingState state) {    }
    ...
    switch (state) {  }
      ...
      case ProcessingState.loading:    return await _mutexManager.withLock('audioSourceCache', () async {

        return AudioProcessingState.loading;      _audioSourceCache.clear();
      ...
      case ProcessingState.ready:  
        ...
        return AudioProcessingState.ready;  Future<void> _setUserIntentAtomic(bool intendedPlaying) async {

      case ProcessingState.completed:    return await _mutexManager.withLock('userIntent', () async {

        return AudioProcessingState.completed;      if (kDebugMode) {
  ...
  void _updateCurrentMediaItem() {      _userIntendedPlaying = intendedPlaying;

    if (_currentIndex != null &&       

        _currentIndex! >= 0 &&       // Clear explicit pause flag when user intends to play

        _currentIndex! < _playlist.length) {      if (intendedPlaying) {

      final track = _playlist[_currentIndex!];        _userExplicitlyPaused = false;
  ...
  void _updateCustomState() {      // Update state machine intent

    _customStateController.add({      if (intendedPlaying) {

      'shuffle': _shuffleEnabled,        _stateMachine.setIntent(UserIntent.play);

      'repeat': _repeatMode.index,      } else {

      'currentIndex': _currentIndex,        _stateMachine.setIntent(UserIntent.pause);
  ...
  void _handlePlaybackError() {        final verificationDelay = isMobile ? 25 : 100; // Reduce mobile delay from 100ms to 25ms
    ...
    if (kDebugMode) {        
      ...
      print('DoudouAudioHandler: Handling playback error, attempting recovery');        if (kDebugMode && isMobile) {
    ...
    Future.delayed(const Duration(seconds: 1), () async {        }
      ...
      if (_currentIndex != null && _currentIndex! < _playlist.length) {        
        ...
        try {        Future.delayed(Duration(milliseconds: verificationDelay), () {
  ...
  /// Create a MediaItem from a Track  /// Verifies that the actual player state aligns with user intent and corrects if needed

  MediaItem _createMediaItem(Track track) {  Future<void> _verifyUserIntentAlignment() async {
    ...
    return MediaItem(    // Skip verification during transitions to avoid interference
      ...
      title: track.name,      return;

      artist: track.artistName ?? 'Unknown Artist',    }

      album: track.albumName ?? 'Unknown Album',    

      duration: track.duration != null ? Duration(milliseconds: track.duration!) : null,    final playerPlaying = _player.playing;

      artUri: track.imageUrl != null ? _buildImageUri(track.imageUrl!) : null,    final userWantsPlaying = _userIntendedPlaying && !_userExplicitlyPaused;

      playable: true,    final processingState = _player.processingState;

      extras: {    

        'trackId': track.id,    // Only verify for ready state to avoid interfering with loading/buffering

        'albumId': track.albumId,    if (processingState != ProcessingState.ready) {

        'trackNumber': track.trackNumber,      return;
  ...
  Uri? _buildImageUri(String imageId) {      }
    ...
    try {      
      ...
      // Use JellyfinService to build proper image URL      try {

      final imageUrl = _jellyfinService.getImageUrl(imageId, ImageType.primary);        if (userWantsPlaying && !playerPlaying) {

      return Uri.parse(imageUrl);          if (kDebugMode) {

    } catch (e) {            print('Correcting: Starting playback to align with user intent');

      if (kDebugMode) {          }

        print('DoudouAudioHandler: Failed to build image URI: $e');          await _player.play();
  ...
  Future<String?> _getStreamUrl(Track track) async {      } catch (e) {
    ...
    try {        if (kDebugMode) {
      ...
      if (kDebugMode) {      final now = DateTime.now();

        print('DoudouAudioHandler: Failed to get stream URL for ${track.name}: $e');      if (_lastBufferingTime != null && 

      }          now.difference(_lastBufferingTime!) < const Duration(seconds: 10)) {

      return null;        _bufferingLoopCount++;
  ...
  Future<void> _playTrackAtIndex(int index) async {    });
    ...
    try {  Future<bool> _shouldHandleCodecLoop() async {

      _currentIndex = index;    return await _mutexManager.withLock('bufferingState', () async {

      final track = _playlist[index];      return _bufferingLoopCount >= 25;
    ...
    return await _mutexManager.withLock('bufferingState', () async {
      ...
      final streamUrl = await _getStreamUrl(track);      _lastBufferingTime = null;
  ...
  // Helper method to completely clear and reset player state
      ...
      if (streamUrl.startsWith('file://')) {    return await _playerOperationQueue.enqueue('resetPlayerState', () async {

        await _player.setFilePath(streamUrl.substring(7));      _logger.info('Completely resetting player state', 'AudioHandler');

      } else {      

        await _player.setUrl(streamUrl);      // Set flag to prevent recovery interference during intentional reset

      }      _isIntentionallyResetting = true;

      

      // Update media item and state      try {
      ...
      _updatePlaybackState();        final stopDelay = isMobile ? 150 : 200; // Conservative mobile delay for MediaCodec stability

        final finalDelay = isMobile ? 200 : 300; // Conservative mobile delay for MediaCodec stability

      if (kDebugMode) {        

        print('DoudouAudioHandler: Successfully loaded track: ${track.name}');        if (kDebugMode && isMobile) {

      }          if (kDebugMode) {

    } catch (e) {            print('Mobile optimized reset: Using conservative delays (${stopDelay}ms + ${finalDelay}ms)');
      ...
      // Update state to show error      await _errorStateManager.executeWithErrorHandling(

      playbackState.add(playbackState.value.copyWith(        component: 'AudioHandler',
