...
class CacheService {
        ...
        if (oldVersion < 2) {
          ...
          try {
            final result = await db.rawQuery(
              "PRAGMA table_info(album_tracks_cache)",
            );
            ...
            if (hasOldSchema) {
              ...
              await db.execute(
                'ALTER TABLE album_tracks_cache_new RENAME TO album_tracks_cache',
              );
            }
          } catch (e) {
            // album_tracks_cache table does not exist or migration failed
          }

          // Check if playlist_tracks_cache exists with old schema and migrate
          try {
            final result = await db.rawQuery(
              "PRAGMA table_info(playlist_tracks_cache)",
            );
            bool hasOldSchema = false;
            for (final column in result) {
            ...
            if (hasOldSchema) {
              ...
              await db.execute(
                'ALTER TABLE playlist_tracks_cache_new RENAME TO playlist_tracks_cache',
              );
            }
          } catch (e) {
            // playlist_tracks_cache table does not exist or migration failed
          }
        }
      },
    );
  ...
  Future<void> _setCache(
    ...
    if (kIsWeb) {
      ...
      final cacheData = {'data': data, 'timestamp': now};
      ...
      try {
        ...
        // Error writing to cache table, attempting to recreate with correct schema
        ...
        try {
          ...
          // Failed to cache even after recreating table
        }
      }
    }
  }

  Future<Map<String, dynamic>?> _getCache(
    String table,
    String key,
    Duration maxAge,
  ) async {
    if (kIsWeb) {
      ...
      try {
        ...
        // Error reading from SharedPreferences cache
        return null;
      }
    } else {
      // Use database for other platforms
      ...
      try {
        ...
        // Error reading from cache table, recreating with correct schema
        await _recreateTableWithCorrectSchema(table);
        return null;
      }
    }
  ...
  Future<void> cacheArtists(List<Artist> artists) async {
    ...
    await _setCache(
      ...
      'all_artists',
      data,
      duration: _artistsCacheDuration,
    );
  }
  ...
  Future<void> cachePlaylists(List<Playlist> playlists) async {
    ...
    await _setCache(
      ...
      'all_playlists',
      data,
      duration: _playlistsCacheDuration,
    );
  }
