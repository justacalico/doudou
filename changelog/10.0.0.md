...
- Fixed Navidrome track durations displaying incorrectly (Subsonic API returns seconds, now properly converted to milliseconds).
- Fixed Navidrome service skipping the 2nd song in queue.
- Fixed Navidrome favoriting not persisting (track refresh was replacing starred tracks with new random songs).
